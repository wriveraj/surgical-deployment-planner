...
A relational database project simulating a surgical instrument and implant deployment operation across six sites. Built to demonstrate real-world field inventory planning, Advanced Allocation workflows, inspection-based readiness tracking, and restock flag management using PostgreSQL.
...
I manage field inventory for a medical device division. The core of that work is **Advanced Allocation**, a pre-planning process where implants are assigned to specific surgeons, procedures, and sites days before the case. A missed allocation is high-risk when a surgical procedure depends on it. This project models that workflow on fully synthetic data.

The database tracks every stage of the implant lifecycle: deployment to the field, retrieval, integrity inspection, and redeployment, with a 7-day planning window and mandatory 24-hour buffer gaps between cases.
...
| `locations` | 6 sites + 1 central warehouse |
...
- **Tibial Baseplate at SITE-01** - failed inspection with a confirmed case in the window. Highest priority restock.
- **Acetabular Cup at SITE-02** - inspection still pending for a confirmed case. Risk of same-day scramble.
- **Radial Head at SITE-04** - Urgent case with no inspection completed yet.
